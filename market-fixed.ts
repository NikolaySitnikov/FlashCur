...
market.get('/data', async (c) => {
    try {
        // Gracefully handle unauthenticated requests (for development)
        let user, tier = 'free'
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                logger.info('Market data request without auth (development mode)')
                user = { id: 'dev-user', email: 'dev@volspike.com', tier: 'free' } as any
                tier = 'free'
            } else {
                // In production, require authentication
                logger.warn('Market data request without auth in production')
